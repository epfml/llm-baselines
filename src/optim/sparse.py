...
from .utils import eval_sparse, get_batch, eval_sweep_dropk
        ...
        if itr % eval_freq == 0 or itr == iterations: # eval: from here it's pretty much only evaluation, all the training is above 
            ...
            if extra_args.wandb:
                ...
                if itr % (eval_freq * 5) == 0 or itr == iterations:
                    ...
                    wandb.log({f"val-stats/sparsity-{wandb.run.name}" : wandb.plot.line_series(xs=layer_idx, ys=sparsity_plot, keys=keys, title=f"sparsity-{wandb.run.name}")})
                ...
                if itr % (eval_freq * 50) == 0 or itr == iterations:
                    ...
                    wandb.log({f"val-stats/Acc-drop-k-itr={itr}" : wandb.plot.line(table_acc, "n heads dropped", "acc", title=f"Acc: drop-k-itr={itr}")})
                    table_pp = wandb.Table(data=list(zip(x_axis, y_axis_pp)), columns = ["n heads dropped", "perplexity"])
                    wandb.log({f"val-stats/PP-drop-k-itr={itr}" : wandb.plot.line(table_pp, "n heads dropped", "perplexity", title=f"Perplexity: drop-k-itr={itr}")})
                    
                    x_axis, y_axis_acc, y_axis_pp, _ = eval_sweep_dropk(model, data['val'], sequence_length, batch_size, extra_args.n_head, 
                                                                                  extra_args.device, max_num_batches=24, ctx=ctx)
                    table_acc = wandb.Table(data=list(zip(x_axis, y_axis_acc)), columns = ["n heads dropped", "acc"])
                    wandb.log({f"val-stats/Acc-alpha-th-sweep-itr={itr}" : wandb.plot.line(table_acc, "n heads dropped", "acc", title=f"Acc: alpha-th-sweep-itr={itr}")})
                    table_pp = wandb.Table(data=list(zip(x_axis, y_axis_pp)), columns = ["n heads dropped", "perplexity"])
