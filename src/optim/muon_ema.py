...
"""Implementation of the MuonEMA optimizer."""
...
class MuonEMA(Optimizer):
    ...
    def _init_group(
        ...
        for p in group["params"]:
            ...
            if "fast_momentum_buffer" not in state:
                state["fast_momentum_buffer"] = torch.zeros_like(
                    p.grad, memory_format=torch.preserve_format
                )
            if "slow_momentum_buffer" not in state:
                state["slow_momentum_buffer"] = torch.zeros_like(
                    p.grad, memory_format=torch.preserve_format
                )
                
            fast_momentum_bufs.append(state["fast_momentum_buffer"])
            slow_momentum_bufs.append(state["slow_momentum_buffer"])
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            fast_momentum = group["fast_momentum"]
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        if nesterov:
            ...
            update.add_(slow_buf, alpha=alpha) # alpha is going to be important
