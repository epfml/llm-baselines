...
class DistributedShampoo(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        super(DistributedShampoo, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(DistributedShampoo, self).__setstate__(state)
    ...
    def step(self, closure: Optional[Callable[[], float]] = None) -> Optional[float]:
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if p.grad.is_sparse:
                    raise RuntimeError(
                        "DistributedShampoo does not support sparse gradients"
                    )
                ...
                if not state:
                    ...
                    if p.dim() >= 2:
                        state["preconditioner1"] = torch.eye(
                            p.size(0), device=p.device, dtype=p.dtype
                        )
                        state["preconditioner2"] = torch.eye(
                            p.size(1), device=p.device, dtype=p.dtype
                        )
                    else:
                        state["preconditioner1"] = torch.tensor(
                            1.0, device=p.device, dtype=p.dtype
                        )
                        state["preconditioner2"] = torch.tensor(
                            1.0, device=p.device, dtype=p.dtype
                        )
            ...
            shampoo_decay = self.defaults["shampoo_decay"]
            ...
            # Update parameters using Shampoo preconditioning
            self._update_distributed_shampoo(
                params_with_grad,
                grads,
                exp_avgs,
                exp_avg_sqs,
    ...
    def _update_distributed_shampoo(
        self,
        ...
        """
        Performs an update with Shampoo preconditioning.
        ...
        for i, param in enumerate(params):
            ...
            # Compute step size NOTE: think about another step_size (mb like in the AdEMAMix or SOAP)
            ...
            if grad.dim() >= 2:
                ...
                # inv_pc1 = torch.linalg.pinv(pc1 + torch.eye(pc1.size(0), device=pc1.device, dtype=pc1.dtype) * eps).sqrt()
                # inv_pc2 = torch.linalg.pinv(pc2 + torch.eye(pc2.size(1), device=pc2.device, dtype=pc2.dtype) * eps).sqrt()
...
class AdEMAMixDistributedShampoo(torch.optim.Optimizer):
    """
    ...
    def step(self, closure: Optional[Callable[[], float]] = None) -> Optional[float]:
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if p.grad.is_sparse:
                    raise RuntimeError(
                        "AdEMAMixDistributedShampoo does not support sparse gradients"
                    )
                if not p.requires_grad:
                    continue
        ...
        for i, param in enumerate(params):
            ...
            if T_alpha_beta3 is not None and T_alpha_beta3 > 0:
                ...
                if T_alpha_beta3 != step:
                    ...
                    denominator = (1 - step / T_alpha_beta3) * log_beta3 + (
                        step / T_alpha_beta3
                    ) * log_beta1
                    if denominator != 0:
                        beta3_t = min(
                            math.exp((log_beta1 * log_beta3) / denominator), beta3
                        )
                    else:
                        beta3_t = beta3
                else:
            ...
            combined_grad = (
                exp_avg + alpha_t * exp_avg_slow + preconditioned_grad
            ) / 3  # Weighted average
